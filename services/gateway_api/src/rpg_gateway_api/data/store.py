...
class PostgresDataStore:
    ...
    def _ensure_schema(self) -> None:
        with self._connect() as conn, conn.cursor() as cur:
            ...
            migrations_root = resources.files("rpg_gateway_api.data.migrations")
            migration_files = [p for p in migrations_root.iterdir() if p.suffix == ".sql"]
            for path in sorted(migration_files, key=lambda p: p.name):
